...
describe('PRUEBA DE LA FUNCIONALIDAD DE USUARIOS: HAPPY PATH', () => {
    it('passes', () => {
     ...
     //ESCRIBIR EL USUARIO Y CONTRASEÑA EN EL ACCESO A USUARIOS       
      ...
      cy.get(':nth-child(5) > :nth-child(6) > .btn-primary').should("be.visible").contains("Editar").click()
      ...
      cy.get('#LastName').should("be.visible").type("Riquelme")
       //CONTRASEÑA
       cy.get(':nth-child(4) > .form-label').should("be.visible").contains("Contraseña")
       cy.get('#Password').clear()
       cy.get('#Password').should("be.visible").type("123456")
      //DOCUMENTO
      cy.get(':nth-child(5) > .form-label').should("be.visible").contains("DNI")
      cy.get('#DocumentNumber').clear()
      cy.get('#DocumentNumber').should("be.visible").type("31789941")
      //MAIL
      cy.get(':nth-child(6) > .form-label').should("be.visible").contains("Email")
      cy.get('#Mail').clear()
      cy.get('#Mail').should("be.visible").type("juanRiquelme@hotmail.com.ar")
      //TELEFONO
      cy.get(':nth-child(7) > .form-label').should("be.visible").contains("Telefono")
